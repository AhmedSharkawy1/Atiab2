      ...
      <div className="relative aspect-[16/10] md:aspect-[21/9] rounded-[2.5rem] overflow-hidden mb-6 shadow-2xl border border-zinc-200 dark:border-white/5 bg-zinc-200 dark:bg-zinc-900 reveal-item group">
        ...
        <div className="absolute bottom-8 right-8 left-8 text-right">
          <div className="flex flex-col gap-1">
            <span className="text-yellow-500 font-black text-[11px] tracking-[0.2em] uppercase" aria-hidden="true">فئة القائمة</span>
            <div className="flex items-center gap-3">
              <h2 id={`${section.id}-heading`} className="text-3xl font-black text-white leading-none">{section.title}</h2>
              <span className="text-3xl leading-none animate-emoji" aria-hidden="true">{section.emoji}</span>
            </div>
          </div>
        </div>
      </div>
                ...
                <div className="flex flex-col gap-1 max-w-[50%]">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5" aria-hidden="true">
                      {item.isPopular && (
                        <span className="bg-yellow-500 text-black text-[9px] font-black px-2 py-0.5 rounded-lg animate-popular shadow-md shadow-yellow-500/20">
                          مميز
                        </span>
                      )}
                      {item.isSpicy && (
                        <span className="animate-spicy text-sm leading-none transition-all duration-300">🌶️</span>
                      )}
                    </div>
                  </div>
                </div>
                    ...
                    return (
                      <div key={pIdx} className="flex flex-col items-center gap-1">
                        ...
                        <div className={`
                          ...
                          group-hover:shadow-yellow-600/10 flex items-center justify-center gap-1
                        `}>
                            {isTxt ? (
